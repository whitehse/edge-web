...
async function enterExplain(user) {
  if ($("auth-gate")) setVisible($("auth-gate"), false);
  setVisible($("wrap"), true);
  if ($("topbar")) setVisible($("topbar"), true);
  const b = $("userBadge");
  if (b && user) {
...
async function main() {
  if (window.EdgeShell && EdgeShell.requireAuth) {
    const ok = await EdgeShell.requireAuth();
    if (!ok) return;
  }
  ...
  location.replace("/?next=" + encodeURIComponent("/explain/"));
}
